...
@allure.story('Существующий продавец. Данные компании продавца ООО')
@pytest.mark.all
@pytest.mark.seller
@pytest.mark.company_details


class TestSellerCompanyDetailsLegalOld:

    #Существующий продавец #########################################################################№
    @allure.title('Успешное сохранение Юр.лица в статус Проверен(STATE ACTIVE). Только обязательные поля')
    def test_seller_company_details_legal1(self, page: Page, db_connection, intercept_requests,  photo_paths):  
        ...
        company_details.delete_operator_info(db_connection, 462)
        ###
    ...
    @allure.title('Успешное сохранение Юр.лица в статус Отклонен автомодерацией(STATE LIQUIDATED) Только обязательные поля')
    def test_seller_company_details_legal2(self, page: Page, db_connection,intercept_requests):  
        assertions = AssertionsCompanyDetailsLegal(page)  
        company_details = CompanyDetailsLegal(page)
        # Пока обнуляю продавца и перед стартом тестов, на всякий случай
        company_details.update_operator_status(db_connection, SELLER_LOGIN3_COMPANYDETAILS, 0)
    ...
    @allure.title('Успешное сохранение Юр.лица в статус Отклонен автомодерацией(STATE NULL Неизвестный статус) Только обязательные поля')
    def test_seller_company_details_legal3(self, page: Page, db_connection,intercept_requests):  
        assertions = AssertionsCompanyDetailsLegal(page)  
        company_details = CompanyDetailsLegal(page)
        # Пока обнуляю продавца и перед стартом тестов, на всякий случай
        company_details.update_operator_status(db_connection, SELLER_LOGIN3_COMPANYDETAILS, 0)
    ...
    @allure.title('Успешное сохранение Юр.лица в статус Проверен(STATE ACTIVE). Все поля')
    def test_seller_company_details_legal4(self, page: Page, db_connection, intercept_requests,  photo_paths):  
        assertions = AssertionsCompanyDetailsLegal(page)  
        company_details =CompanyDetailsLegal(page)
        # Пока обнуляю продавца и перед стартом тестов, на всякий случай
        company_details.update_operator_status(db_connection, SELLER_LOGIN3_COMPANYDETAILS, 0)
        ...
        assertions.check_operator_info__photo_db(db_connection, SELLER_LOGIN3_COMPANYDETAILS, 10)
        assertions.check_operator_logo_in_db(db_connection,SELLER_LOGIN3_COMPANYDETAILS, False)
        paths = photo_paths( "avatar.webp")
        company_details.upload_photos_company('Инфо организатора поле загрузки аватара', *paths)
        assertions.check_operator_logo_in_db(db_connection, SELLER_LOGIN3_COMPANYDETAILS, True)
        company_details.click_element('кнопка Сохранить в данных компании продавца')
        assertions.check_operator_status_front( page, 'Проверен')
        assertions.check_operator_status(db_connection, SELLER_LOGIN3_COMPANYDETAILS, 2)
        assertions.check_request_statuses(intercept_requests)
    ...
    @allure.title('Проверка вывода ошибок')
    def test_seller_company_details_legal5(self, page: Page, db_connection,intercept_requests):  
        assertions = AssertionsCompanyDetailsLegal(page)  
        company_details = CompanyDetailsLegal(page)
        # Пока обнуляю продавца и перед стартом тестов, на всякий случай
        company_details.update_operator_status(db_connection, SELLER_LOGIN3_COMPANYDETAILS, 0)
        ...
        #Информация для путешественников
        company_details.fill_element('Инфо организатора имя', SELLER_LOGIN3_COMPANYDETAILS)
    ...
    @allure.title('Проверка, что не сохраняются данные без обязательных полей')
    def test_seller_company_details_legal6(self, page: Page, db_connection,intercept_requests):  
        assertions = AssertionsCompanyDetailsLegal(page)  
        company_details =CompanyDetailsLegal(page)
        # Пока обнуляю продавца и перед стартом тестов, на всякий случай
        company_details.update_operator_status(db_connection, SELLER_LOGIN3_COMPANYDETAILS, 0)
        ...
        assertions.check_legal_in_db(db_connection, 
                                    ...
                                    None,
                                    None,
                                    'Крокодилов Крокодил Крокодилович',
                                    'Крокодилов Крокодил Крокодилович',
                                    None,
                                    None,
                                    None,
    ...
    @allure.title('Проверка, что подтягивается из DADATA данные в Юр')
    def test_seller_company_details_legal7(self, page: Page, db_connection, intercept_requests):  
        assertions = AssertionsCompanyDetailsLegal(page)  
        company_details = CompanyDetailsLegal(page)
        # Пока обнуляю продавца и перед стартом тестов, на всякий случай
        company_details.update_operator_status(db_connection, SELLER_LOGIN3_COMPANYDETAILS, 0)
